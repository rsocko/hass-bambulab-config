...
**File:** `dashboards/templates.yaml`  
...
3. **Automation alert**: `dashboards/smart-status-unmapped-alert.yaml` triggers a persistent notification whenever the sensor enters `Unmapped Printer State`, with a 30-second debounce, system log entry, and auto-dismiss when the state clears.

To add support for a new value, either:
- Add it to the relevant display branch in `dashboards/templates.yaml`, **and** add it to `known_stages` in the `detail` block.
