...
def test_build_model_detail_response_local_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(models_router, "read_model_project_memberships_bulk", lambda **_kwargs: {"local-model-1": []})
    ...
    monkeypatch.setattr(models_router, "_read_idea_metadata", lambda **_kwargs: None)
    monkeypatch.setattr(models_router, "read_archive_links_for_model", lambda **_kwargs: [])
    monkeypatch.setattr(
        models_router,
...
def test_build_model_detail_response_local_success_with_route_helper_map(monkeypatch) -> None:
    from sidecars.model_catalog.app.routers import models as models_router
    ...
    monkeypatch.setattr(models_router, "_read_idea_metadata", lambda **_kwargs: None)
    monkeypatch.setattr(models_router, "read_archive_links_for_model", lambda **_kwargs: [])
    monkeypatch.setattr(models_router, "_ranking_payload", lambda ranking: ranking)
    ...
    assert payload["success"] is True
    assert payload["model"]["project_ids"] == []
    assert payload["enrichment"]["project_memberships"] == []
...
def test_build_model_detail_response_rejects_non_local_authority(monkeypatch) -> None:
    from sidecars.model_catalog.app.routers import models as models_router
    ...
    assert payload["success"] is False
    assert payload["error"] == "model_not_found"
    assert payload["message"] == "Non-local model authorities are no longer supported."
...
def test_model_detail_helpers_include_idea_metadata_dependency() -> None:
    ...
    assert "read_model_project_memberships_bulk" in helpers
