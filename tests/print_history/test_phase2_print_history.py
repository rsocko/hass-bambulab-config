...
class TestHeatmapActivityCard(unittest.TestCase):
    ...
    def test_photo_gallery_uses_top_left_advanced_actions_menu_and_delete_confirmations(self):
        ...
        self.assertIn('action === "timelapse"', content)
        ...
        self.assertIn('type: "custom:print-history-timelapse-editor-card"', content)
        self.assertIn('_buildArchiveTimelapseEditorCardConfig(archive)', content)
        self.assertIn('action === "advanced-actions"', browser_content)
        self.assertIn('action === "timelapse"', browser_content)
        self.assertIn('data-action="timelapse"', browser_content)
        self.assertIn('icon="mdi:movie-open-play-outline"', browser_content)
        ...
        self.assertIn('_buildArchiveTimelapseEditorCardConfig(archive)', browser_content)
        ...
        self.assertIn('var detailArchive = this._parseJson(detailArchiveRaw || "{}", {});', action_content)
        self.assertIn('var detailStorage = this._parseJson(detailStorageRaw || "{}", {});', action_content)
        self.assertIn('_parseJson(value, fallbackValue)', action_content)
        ...
        self.assertIn('analytics-overview', action_content)
        self.assertIn('Tracked Storage', action_content)
        self.assertIn('Duplicate Lineage', action_content)
        self.assertIn('@media (max-width: 700px)', action_content)
        self.assertIn('@media (max-width: 900px)', action_content)
        ...
        self.assertIn('Scan Printer for Timelapse', action_content)
        ...
        self.assertIn('_storageMetricsNeedForceRefresh(archive)', action_content)
        self.assertIn('return timelapseBytes <= 0 || !artifactPath;', action_content)
        ...
        self.assertIn('scan_timelapse', action_content)
        ...
        self.assertIn('/api/bambuddy/print-history/archive/{archive_id}/timelapse/upload', action_content)
        self.assertIn('this.shadowRoot.addEventListener("change", this._boundSourceUploadChangeHandler);', action_content)
        self.assertIn('input.id !== "source-upload-input" && input.id !== "timelapse-upload-input"', action_content)
    ...
    def test_timelapse_cards_use_proxy_routes_and_processing_event(self):
        ...
        self.assertIn('_fireBrowserModEvent("browser_mod.sequence", {', viewer_content)
        self.assertIn('service: "browser_mod.close_popup"', viewer_content)
        ...
        self.assertIn('Replace Current Timelapse', editor_content)
        self.assertIn('window.addEventListener("bambuddy-print-history-archive-updated", this._boundArchiveUpdatedHandler);', editor_content)
        self.assertIn('this._archiveOverride = Object.assign({}, currentArchive, updatedArchive);', editor_content)
        self.assertNotIn('Save As New File', editor_content)
        self.assertIn('appearance:none', editor_content)
        self.assertIn('color-scheme:light dark', editor_content)
        self.assertIn('does not create alternate archive video files', editor_content)
    ...
    def test_heatmap_card_supports_filaments_used_label(self):
        ...
        self.assertIn('input.mode === "Storage Used"', content)
        ...
        self.assertIn('"storage used": "Storage Used"', content)
    ...
    def test_heatmap_tooltip_prioritizes_selected_metric_without_duplicate_rows(self):
        ...
        self.assertIn("{ key: 'Storage Used', label: 'Storage', value: this._formatBytes(meta.storageBytes || 0) }", content)
    ...
    def test_heatmap_card_formats_large_totals_with_locale_grouping(self):
        ...
        self.assertIn('return this._formatTrimmedDecimal(value, 1) + " " + units[unitIndex];', content)
    ...
    def test_activity_panel_switches_filament_weight_icon_to_kg_when_needed(self):
        ...
        self.assertIn("'Storage Used': 'mdi:harddisk'", content)
...
class TestPrintHistoryTagEditorCard(unittest.TestCase):
    ...
    def test_archive_restore_card_registration_is_guarded(self):
        ...
        self.assertIn('return String(', content)
    ...
    def test_replacement_upload_endpoint_can_auto_assign_sole_printer(self):
        content = (ROOT / "homeassistant" / "custom_components" / "bambuddy" / "__init__.py").read_text("utf-8")
        self.assertIn('async def _async_resolve_archive_printer_id(', content)
        self.assertIn('operation="assign_archive_printer_for_replacement_upload",', content)
        self.assertIn('raise HomeAssistantError("Archive printer is missing and no sole configured printer could be inferred")', content)
        self.assertIn('"auto_assigned_printer_id": auto_assigned_printer_id,', content)
    ...
    def test_archive_actions_card_includes_related_and_compare_modes(self):
        ...
        self.assertIn('type: "bambuddy/print_history_archive_duplicates"', script)
        ...
        self.assertIn('this._renderActionButton("open-duplicates", "Duplicates", "mdi:content-copy"', script)
        ...
        self.assertIn('this._mode === "duplicates"', script)
        ...
        self.assertIn('String(row && row.field || "") === "content_hash"', script)
        self.assertIn('.compare-hash{font-family:ui-monospace', script)
        self.assertIn('Use Related Prints for broader similarity matches, Duplicates for explicit Bambuddy duplicate families', script)
        ...
        self.assertIn('Source Print', script)
        self.assertIn('Duplicate Copies', script)
        self.assertIn('This view uses Bambuddy\'s explicit duplicate lineage fields', script)
    ...
    def test_archive_actions_card_routes_metadata_correction_through_repair_chooser(self):
        ...
        self.assertIn('if (Array.isArray(payload) && payload.length) {', script)
