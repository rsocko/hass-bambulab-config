...
class ModelCatalogBrowserCard extends HTMLElement {
  constructor() {
    ...
    this._bulkProjectMemberStatePending = false;
  ...
  async _bulkSetProjectMemberState(memberState) {
    var nextState = String(memberState || '').trim().toLowerCase();
    ...
    if (!projectId || !project || String(project.status || '').trim().toLowerCase() !== 'evaluating' || !selectedRefs.length || this._bulkProjectMemberStatePending) {
      return;
    }
    if (this._projectMemberStateOptions().indexOf(nextState) === -1) {
      return;
    }
    this._bulkProjectMemberStatePending = true;
    this._error = '';
    this._render();
    var updatedCount = 0;
    var skippedCount = 0;
    ...
    try {
      for (var index = 0; index < selectedRefs.length; index++) {
        ...
        try {
          ...
          var found = false;
          var changed = false;
          var nextMemberships = memberships.map(function (membership) {
            var item = membership && typeof membership === 'object' ? membership : {};
            var membershipProjectId = parseInt(String(item.project_id || '0'), 10);
            var currentState = String(item.member_state || 'candidate').trim().toLowerCase() || 'candidate';
            if (membershipProjectId === projectId) {
              found = true;
          ...
          await this._replaceModelProjectMemberships(modelRef, nextMemberships);
          this._setLocalProjectMemberState(projectId, modelRef, nextState);
          updatedCount += 1;
        } catch (_bulkError) {
          failedCount += 1;
        }
      ...
      var feedback = '';
      if (updatedCount > 0) {
        feedback = 'Updated ' + String(updatedCount) + (updatedCount === 1 ? ' project member' : ' project members') + ' to ' + this._projectMemberStateLabel(nextState) + '.';
        if (skippedCount > 0) {
          feedback += ' ' + String(skippedCount) + ' already matched.';
        }
        if (failedCount > 0) {
          feedback += ' ' + String(failedCount) + ' failed.';
        }
        this._setCollectionActionFeedback(feedback, failedCount > 0 ? 'error' : 'success');
      } else if (skippedCount > 0 && failedCount === 0) {
        this._setCollectionActionFeedback('Selected project members already matched ' + this._projectMemberStateLabel(nextState) + '.', 'success');
      } else if (failedCount > 0) {
        this._error = 'Could not update selected project members.';
      }
      if (updatedCount > 0) {
  ...
  async _fetchDeletedModelsPage(requestPayload) {
    var base = String(this._resolveModelSidecarUrl() || "").trim().replace(/\/$/, "");
    if (!base) {
      throw new Error("No sidecar URL configured");
    }
    var page = Math.max(1, Number(requestPayload && requestPayload.page || 1) || 1);
    var perPage = Math.max(1, Number(requestPayload && requestPayload.per_page || this._pagination.per_page || 12) || 12);
    var params = new URLSearchParams();
    params.set("limit", String(perPage));
    params.set("offset", String((page - 1) * perPage));
    if (requestPayload && requestPayload.q) {
      params.set("q", String(requestPayload.q));
    }
    var response = await fetch(base + "/api/local/deleted-models?" + params.toString(), {
      method: "GET",
      headers: Object.assign({ "Accept": "application/json" }, await this._authHeaders(false)),
      credentials: "omit",
    });
    if (response.status === 401) {
      response = await fetch(base + "/api/local/deleted-models?" + params.toString(), {
        method: "GET",
    ...
    if (!response.ok) {
      throw new Error("Failed to load deleted models (" + response.status + ")");
    }
    var payload = await response.json();
    var entriesById = {};
    var entries = Array.isArray(payload && payload.entries) ? payload.entries : [];
    for (var i = 0; i < entries.length; i++) {
      var entryId = String(entries[i] && entries[i].local_model_id || "").trim();
      if (entryId) {
        entriesById[entryId] = entries[i];
      }
    }
    var models = (Array.isArray(payload && payload.models) ? payload.models : []).map(function (model) {
      var item = Object.assign({}, model || {});
      var localModelId = String(item.public_id || item.local_model_id || "").trim();
      var entry = entriesById[localModelId] || {};
      item.local_model_id = localModelId;
      item.authority = "local";
      item.source_authority = "local";
      item.deleted_at = entry.deleted_at || item.deleted_at || null;
      item.linked_archive_count = Number(item.linked_archive_count || 0) || 0;
      return item;
    });
    var total = Number(payload && payload.pagination && payload.pagination.total || models.length) || 0;
    return {
      results: models,
      filters: { show_deleted: true, q: requestPayload && requestPayload.q || "" },
      pagination: {
        page: page,
        per_page: perPage,
        total: total,
  ...
  _syncFormIntoFilters() {
    ...
    this._filters.show_deleted = !!(root.querySelector("#mc-show-deleted") && root.querySelector("#mc-show-deleted").checked);
  ...
  async _loadPage(page, refresh) {
    ...
    try {
      ...
      var requestPayload = {
        ...
        show_deleted: !!this._filters.show_deleted,
      ...
      var data;
      if (requestPayload.show_deleted) {
        data = await this._fetchDeletedModelsPage(requestPayload);
  ...
  async _handleClick(event) {
    ...
    if (action === "toggle-show-deleted-filter") {
      this._filters.show_deleted = !this._filters.show_deleted;
      if (this._filters.show_deleted) {
        this._filters.show_archived = false;
        this._browserScope = "models";
      }
      this._cancelScheduledApply();
    ...
    if (action === "bulk-restore-models") {
      event.preventDefault();
      event.stopPropagation();
      await this._bulkRestoreSelectedModels();
      return;
    }

    if (action === "bulk-purge-models") {
      event.preventDefault();
      event.stopPropagation();
      await this._bulkPurgeSelectedModels();
      return;
    }
    ...
    if (action === 'set-project-member-state') {
      event.preventDefault();
      event.stopPropagation();
      await this._setProjectMemberState(
        target.getAttribute('data-project-id'),
        target.getAttribute('data-model-ref'),
        target.getAttribute('data-member-state')
      );
      return;
    }
    ...
    if (action === "restore-model") {
      event.preventDefault();
      event.stopPropagation();
      await this._restoreDeletedModel(target.getAttribute("data-local-model-id"), target.getAttribute("data-model-ref"));
      return;
    }

    if (action === "purge-model") {
      event.preventDefault();
      event.stopPropagation();
      await this._purgeDeletedModel(target.getAttribute("data-local-model-id"), target.getAttribute("data-model-ref"), target.getAttribute("data-model-name"));
      return;
    }
  ...
  _projectMemberStateKey(projectId, modelRef) {
    return String(projectId || '').trim() + '::' + String(modelRef || '').trim();
  }

  _pendingProjectMemberState(projectId, modelRef) {
    var key = this._projectMemberStateKey(projectId, modelRef);
    return !!(this._projectMemberStatePending && this._projectMemberStatePending[key]);
  }

  _projectMemberStateForModel(project, modelRef) {
    var targetRef = String(modelRef || '').trim();
    var memberships = Array.isArray(project && project.model_memberships) ? project.model_memberships : [];
    for (var index = 0; index < memberships.length; index++) {
      var membership = memberships[index] && typeof memberships[index] === 'object' ? memberships[index] : {};
      if (String(membership.model_ref || '').trim() === targetRef) {
        return String(membership.member_state || 'candidate').trim().toLowerCase() || 'candidate';
      }
    }
    return 'candidate';
  }

  _setLocalProjectMemberState(projectId, modelRef, memberState) {
    var normalizedProjectId = parseInt(String(projectId || '0'), 10);
    var targetRef = String(modelRef || '').trim();
    var normalizedState = String(memberState || 'candidate').trim().toLowerCase() || 'candidate';
    if (!Number.isFinite(normalizedProjectId) || normalizedProjectId <= 0 || !targetRef) {
      return;
    }
    var detail = this._projectDetail && typeof this._projectDetail === 'object' ? this._projectDetail : null;
    var project = detail && detail.project && typeof detail.project === 'object' ? detail.project : null;
    if (project && Number(project.id || 0) === normalizedProjectId && Array.isArray(project.model_memberships)) {
      for (var membershipIndex = 0; membershipIndex < project.model_memberships.length; membershipIndex++) {
        var membership = project.model_memberships[membershipIndex] && typeof project.model_memberships[membershipIndex] === 'object'
          ? project.model_memberships[membershipIndex]
          : null;
        if (membership && String(membership.model_ref || '').trim() === targetRef) {
          membership.member_state = normalizedState;
          break;
        }
      }
    }

    var updateMembershipRows = function (model) {
      var item = model && typeof model === 'object' ? model : null;
      if (!item || !Array.isArray(item.project_memberships)) {
        return;
      }
      for (var itemIndex = 0; itemIndex < item.project_memberships.length; itemIndex++) {
        var row = item.project_memberships[itemIndex] && typeof item.project_memberships[itemIndex] === 'object'
          ? item.project_memberships[itemIndex]
          : null;
        if (row && Number(row.project_id || 0) === normalizedProjectId) {
          row.member_state = normalizedState;
        }
      }
    };

    if (detail && Array.isArray(detail.items)) {
      for (var detailIndex = 0; detailIndex < detail.items.length; detailIndex++) {
        var detailItem = detail.items[detailIndex] && typeof detail.items[detailIndex] === 'object' ? detail.items[detailIndex] : null;
        if (detailItem && this._modelRef(detailItem) === targetRef) {
          updateMembershipRows(detailItem);
          break;
        }
      }
    }

    var rootModel = this._modelByRef(targetRef);
    if (rootModel) {
      updateMembershipRows(rootModel);
    }
  }
  ...
  async _setProjectMemberState(projectId, modelRef, memberState) {
    ...
    var targetRef = String(modelRef || '').trim();
    var nextState = String(memberState || '').trim().toLowerCase();
    if (!Number.isFinite(normalizedProjectId) || normalizedProjectId <= 0 || !targetRef || !nextState) {
      return;
    }
    var detail = this._projectDetail && typeof this._projectDetail === 'object' ? this._projectDetail : null;
    var project = detail && detail.project && typeof detail.project === 'object' ? detail.project : null;
    if (!project || String(project.status || '').trim().toLowerCase() !== 'evaluating') {
      return;
    }
    if (this._pendingProjectMemberState(normalizedProjectId, targetRef)) {
      return;
    }

    var currentState = this._projectMemberStateForModel(project, targetRef);
    if (currentState === nextState) {
      return;
    }

    var pendingKey = this._projectMemberStateKey(normalizedProjectId, targetRef);
    this._projectMemberStatePending[pendingKey] = true;
    this._error = '';
    this._render();
    try {
      var memberships = await this._getModelProjectMemberships(targetRef);
      var found = false;
      var nextMemberships = memberships.map(function (membership) {
        var item = membership && typeof membership === 'object' ? membership : {};
        var membershipProjectId = parseInt(String(item.project_id || '0'), 10);
        if (membershipProjectId === normalizedProjectId) {
          found = true;
          return {
            project_id: membershipProjectId,
            member_state: nextState,
        ...
        return {
          project_id: membershipProjectId,
          member_state: String(item.member_state || 'candidate').trim().toLowerCase() || 'candidate',
        };
      });
      if (!found) {
        throw new Error('Project membership not found for this model.');
      }
      await this._replaceModelProjectMemberships(targetRef, nextMemberships);
      this._setLocalProjectMemberState(normalizedProjectId, targetRef, nextState);
      this._setCollectionActionFeedback('Updated project member state to ' + this._projectMemberStateLabel(nextState) + '.', 'success');
    } catch (error) {
      this._error = error && error.message ? String(error.message) : 'Could not update project member state.';
    } finally {
      delete this._projectMemberStatePending[pendingKey];
      this._render();
    }
  }
  ...
  async _bulkDeleteSelectedModels() {
    ...
    var confirmLines = [
      ...
      "",
      "This moves them to Deleted. They are kept indefinitely until you restore or purge them.",
      "Stored model files/assets stay on disk unless you purge from Deleted.",
    ...
    try {
      ...
      try {
        await this._hass.callService("persistent_notification", "create", {
          ...
          message: failedRefs.length ? this._error : "Moved " + String(deletedRefs.length) + " selected model" + (deletedRefs.length === 1 ? "" : "s") + " to Deleted.",
  ...
  _selectedLocalModelTargets() {
    var selectedRefs = this.getSelectedModelRefs();
    var targets = [];
    for (var i = 0; i < selectedRefs.length; i++) {
      var modelRef = selectedRefs[i];
      var model = this._modelByRef(modelRef);
      var localModelId = this._localModelIdForModel(model) || String(modelRef || "").trim();
      if (model && localModelId) {
        targets.push({ model_ref: modelRef, local_model_id: localModelId, model_name: String(model.name || model.model_name || localModelId) });
      }
    }
    return targets;
  }

  async _bulkRestoreSelectedModels() {
    var targets = this._selectedLocalModelTargets();
    if (!targets.length || this._loading) {
      return;
    }
    if (!window.confirm("Restore " + String(targets.length) + " selected deleted model" + (targets.length === 1 ? "" : "s") + " to the active catalog?")) {
      return;
    }
    var restoredRefs = [];
    var failedRefs = [];
    ...
    for (var i = 0; i < targets.length; i++) {
      try {
        await this._restoreLocalModelRequest(targets[i].local_model_id);
        restoredRefs.push(targets[i].model_ref);
      } catch (_error) {
        failedRefs.push(targets[i].model_ref);
      }
    }
    if (restoredRefs.length) {
      this._removeDeletedModelsFromCurrentView(restoredRefs);
      this._pagination.total = Math.max(0, Number(this._pagination.total || 0) - restoredRefs.length);
      this._pagination.total_pages = Math.max(1, Math.ceil(this._pagination.total / (this._pagination.per_page || 12)));
    }
    this._selectedModelRefs.clear();
    for (var j = 0; j < failedRefs.length; j++) {
      this._selectedModelRefs.add(failedRefs[j]);
    }
    this._multiSelectMode = failedRefs.length > 0;
    this._notifySelectionChanged();
    this._loading = false;
    this._activeActionMenu = "";
    if (failedRefs.length) {
      this._error = "Restored " + String(restoredRefs.length) + " selected model" + (restoredRefs.length === 1 ? "" : "s") + "; " + String(failedRefs.length) + " failed.";
    }
    this._requestLoad(this._currentPage(), true);
    this._render();
  }

  async _bulkPurgeSelectedModels() {
    var targets = this._selectedLocalModelTargets();
    if (!targets.length || this._loading) {
      return;
    }
    var count = targets.length;
    if (!window.confirm("Permanently purge " + String(count) + " selected deleted model" + (count === 1 ? "" : "s") + "?\n\nThis removes catalog records and deletes stored local model files/assets. This cannot be undone.\n\nContinue?")) {
      return;
    }
    if (window.prompt("Type PURGE to permanently delete " + String(count) + " selected deleted model" + (count === 1 ? "." : "s."), "") !== "PURGE") {
      return;
    }
    var purgedRefs = [];
    var failedRefs = [];
    ...
    for (var i = 0; i < targets.length; i++) {
      try {
        await this._purgeLocalModelRequest(targets[i].local_model_id);
        purgedRefs.push(targets[i].model_ref);
      } catch (_error) {
        failedRefs.push(targets[i].model_ref);
      }
    }
    if (purgedRefs.length) {
      this._removeDeletedModelsFromCurrentView(purgedRefs);
      this._pagination.total = Math.max(0, Number(this._pagination.total || 0) - purgedRefs.length);
      this._pagination.total_pages = Math.max(1, Math.ceil(this._pagination.total / (this._pagination.per_page || 12)));
    }
    this._selectedModelRefs.clear();
    for (var j = 0; j < failedRefs.length; j++) {
    ...
    this._activeActionMenu = "";
    if (failedRefs.length) {
      this._error = "Purged " + String(purgedRefs.length) + " selected model" + (purgedRefs.length === 1 ? "" : "s") + "; " + String(failedRefs.length) + " failed.";
    }
    this._requestLoad(this._currentPage(), true);
    this._render();
  }
  ...
  async _deleteModel(modelRef, modelName) {
    ...
    var warningLines = [
      ...
      "• The model moves to Deleted and is removed from the active catalog",
      "• It is kept indefinitely until restored or purged",
      "• Stored model files and assets stay on disk unless purged",
    ];
  ...
  async _restoreDeletedModel(localModelId, modelRef) {
    var resolvedLocalId = String(localModelId || "").trim();
    var resolvedRef = String(modelRef || resolvedLocalId || "").trim();
    if (!resolvedLocalId) {
      this._error = "Could not identify deleted local model to restore.";
      this._render();
      return;
    }
    try {
      ...
      this._error = "";
      this._render();
      await this._restoreLocalModelRequest(resolvedLocalId);
      this._removeDeletedModelsFromCurrentView([resolvedRef]);
      this._pagination.total = Math.max(0, Number(this._pagination.total || 0) - 1);
      this._pagination.total_pages = Math.max(1, Math.ceil(this._pagination.total / (this._pagination.per_page || 12)));
      this._loading = false;
      this._activeActionMenu = "";
      ...
      this._loading = false;
      this._error = error && error.message ? String(error.message) : "Failed to restore model";
      this._render();
    }
  }

  async _purgeDeletedModel(localModelId, modelRef, modelName) {
    var resolvedLocalId = String(localModelId || "").trim();
    var resolvedRef = String(modelRef || resolvedLocalId || "").trim();
    var name = String(modelName || "this model").trim() || "this model";
    if (!resolvedLocalId) {
      this._error = "Could not identify deleted local model to purge.";
      this._render();
      return;
    }
    if (!window.confirm("Permanently purge " + name + "?\n\nThis removes the catalog record and deletes stored local model files/assets. This cannot be undone.\n\nContinue?")) {
      return;
    }
    if (window.prompt("Type PURGE to permanently delete " + name + ".", "") !== "PURGE") {
      return;
    }
    try {
      this._loading = true;
      this._error = "";
      this._render();
      await this._purgeLocalModelRequest(resolvedLocalId);
      this._removeDeletedModelsFromCurrentView([resolvedRef]);
      this._pagination.total = Math.max(0, Number(this._pagination.total || 0) - 1);
      this._pagination.total_pages = Math.max(1, Math.ceil(this._pagination.total / (this._pagination.per_page || 12)));
      this._loading = false;
      ...
      this._loading = false;
      this._error = error && error.message ? String(error.message) : "Failed to purge model";
      this._render();
    }
  }

  async _restoreLocalModelRequest(localModelId) {
    var sidecarUrl = this._resolveModelSidecarUrl();
    if (!sidecarUrl) {
      throw new Error("Model Catalog sidecar URL not configured");
    }
    var response = await fetch(sidecarUrl + "/api/local/models/" + encodeURIComponent(localModelId) + "/restore", {
      method: "POST",
      headers: Object.assign({ "Content-Type": "application/json" }, await this._authHeaders(false)),
      credentials: "omit",
    });
    if (response.status === 401) {
      response = await fetch(sidecarUrl + "/api/local/models/" + encodeURIComponent(localModelId) + "/restore", {
        method: "POST",
        headers: Object.assign({ "Content-Type": "application/json" }, await this._authHeaders(true)),
        credentials: "omit",
      });
    ...
    if (!response.ok) {
      throw new Error("Restore failed: HTTP " + String(response.status));
    }
    var result = await response.json();
    if (!result.success) {
  ...
  async _purgeLocalModelRequest(localModelId) {
    var sidecarUrl = this._resolveModelSidecarUrl();
    if (!sidecarUrl) {
      throw new Error("Model Catalog sidecar URL not configured");
    }
    var response = await fetch(sidecarUrl + "/api/local/models/" + encodeURIComponent(localModelId) + "/purge", {
      method: "DELETE",
      headers: Object.assign({ "Content-Type": "application/json" }, await this._authHeaders(false)),
      credentials: "omit",
    });
    if (response.status === 401) {
      response = await fetch(sidecarUrl + "/api/local/models/" + encodeURIComponent(localModelId) + "/purge", {
        method: "DELETE",
        headers: Object.assign({ "Content-Type": "application/json" }, await this._authHeaders(true)),
        credentials: "omit",
      });
    ...
    if (!response.ok) {
      throw new Error("Purge failed: HTTP " + String(response.status));
    }
    var result = await response.json();
    if (!result.success) {
  ...
  _localModelIdForModel(model) {
    ...
    var authority = String(item.authority || item.source_authority || "").trim().toLowerCase();
    return String(item.local_model_id || fields.local_model_id || (authority === "local" ? item.public_id : "") || "").trim();
  }
  ...
  _advancedFilterCount() {
    ...
    if (this._filters && this._filters.show_deleted) {
      count += 1;
    }
  ...
  _renderAdvancedFiltersMenu() {
    ...
    return ''
      ...
      + '      <button class="filter-chip toggle-chip' + (this._filters.show_deleted ? ' active danger' : '') + '" type="button" data-action="toggle-show-deleted-filter" aria-pressed="' + (this._filters.show_deleted ? 'true' : 'false') + '">' + this._escapeHtml(showDeletedLabel) + '</button>'
      + '    </div>'
      ...
      + '    <input id="mc-show-deleted" type="checkbox" hidden ' + (this._filters.show_deleted ? 'checked' : '') + '>'
  ...
  _renderProjectDetailMemberCard(project, model) {
    var item = model && typeof model === 'object' ? model : {};
    var projectId = Number(project && project.id || 0) || 0;
    var modelRef = this._modelRef(item);
    var memberState = this._projectMemberStateForModel(project, modelRef);
    var projectStatus = String(project && project.status || 'evaluating').trim().toLowerCase() || 'evaluating';
    var interactive = projectStatus === 'evaluating' && !!modelRef && projectId > 0;
    var pending = interactive && this._pendingProjectMemberState(projectId, modelRef);
    var stateControls = interactive
      ? '<div class="project-member-state-group" role="group" aria-label="Project member state">'
        + this._projectMemberStateOptions().map(function (stateKey) {
            var active = stateKey === memberState;
            return '<button class="project-member-state-btn' + (active ? ' active' : '') + '" type="button" data-action="set-project-member-state" data-project-id="' + this._escapeHtml(String(projectId)) + '" data-model-ref="' + this._escapeHtml(modelRef) + '" data-member-state="' + this._escapeHtml(stateKey) + '" ' + (pending ? 'disabled' : '') + '>' + this._escapeHtml(this._projectMemberStateLabel(stateKey)) + '</button>';
          }.bind(this)).join('')
        + '</div>'
      : '';
    return ''
      + '<div class="project-member-shell">'
      + '  <div class="project-member-toolbar">'
      + '    <span class="selected-filter-chip tag-chip project-member-state-chip">' + this._escapeHtml(this._projectMemberStateLabel(memberState)) + '</span>'
      + (pending ? '<span class="project-member-pending">Saving…</span>' : '')
      + stateControls
      + '  </div>'
      + this._renderModelCard(item)
      + '</div>';
  }

  _projectDetailStateOrder(memberStateCounts) {
    var preferredOrder = ['chosen', 'candidate', 'rejected', 'printed'];
    var source = memberStateCounts && typeof memberStateCounts === 'object' ? memberStateCounts : {};
    var keys = Object.keys(source);
    var ordered = [];
    for (var preferredIndex = 0; preferredIndex < preferredOrder.length; preferredIndex++) {
      var preferredState = preferredOrder[preferredIndex];
      if (keys.indexOf(preferredState) !== -1) {
        ordered.push(preferredState);
      }
    }
    for (var keyIndex = 0; keyIndex < keys.length; keyIndex++) {
      if (ordered.indexOf(keys[keyIndex]) === -1) {
        ordered.push(keys[keyIndex]);
      }
    }
    return ordered;
  }

  _renderProjectDetailGroupSection(project, stateKey, models) {
    var items = Array.isArray(models) ? models : [];
    if (!items.length) {
      return '';
    }
    return ''
      + '<section class="project-detail-group-section">'
      + '  <div class="project-detail-group-header">'
      + '    <div class="project-detail-group-title">' + this._escapeHtml(this._projectMemberStateLabel(stateKey)) + '</div>'
      + '    <div class="project-detail-group-count">' + this._escapeHtml(String(items.length)) + '</div>'
      + '  </div>'
      + '  <div class="project-detail-group-grid">'
      + items.map(function (model) {
          return this._renderProjectDetailMemberCard(project, model);
        }.bind(this)).join('')
      + '  </div>'
      + '</section>';
  }
  ...
  _renderProjectDetailView() {
    ...
    var orderedMemberStates = this._projectDetailStateOrder(memberStateCounts);
    var memberStateHtml = orderedMemberStates.map(function (stateKey) {
    ...
    if (items.length && !activeMemberStateFilter) {
      ...
      for (var itemIndex = 0; itemIndex < items.length; itemIndex++) {
        var groupedModel = items[itemIndex] && typeof items[itemIndex] === 'object' ? items[itemIndex] : null;
        if (!groupedModel) {
          continue;
        }
        var groupedStateKey = this._projectMemberStateForModel(project, this._modelRef(groupedModel));
        if (!groupedItems[groupedStateKey]) {
          groupedItems[groupedStateKey] = [];
        }
        groupedItems[groupedStateKey].push(groupedModel);
      }
      groupedMemberCards = orderedMemberStates.map(function (stateKey) {
        return this._renderProjectDetailGroupSection(project, stateKey, groupedItems[stateKey] || []);
      }.bind(this)).join('');
    }
    var memberCards = items.length
      ? (groupedMemberCards || items.map(function (model) {
          return this._renderProjectDetailMemberCard(project, model);
        }.bind(this)).join(''))
  ...
  _renderMultiSelectStrip(extraClass) {
    ...
    var detail = this._projectDetail && typeof this._projectDetail === 'object' ? this._projectDetail : null;
    var detailProject = detail && detail.project && typeof detail.project === 'object' ? detail.project : null;
    var projectBulkButtons = '';
    if (this._projectDetailActive() && detailProject && String(detailProject.status || '').trim().toLowerCase() === 'evaluating') {
      var projectBulkDisabled = (!count || this._bulkProjectMemberStatePending) ? ' disabled' : '';
      projectBulkButtons = ''
        + '  <button class="bulk-btn" type="button" data-action="bulk-set-project-member-state" data-member-state="chosen"' + projectBulkDisabled + '>Mark Chosen</button>'
        + '  <button class="bulk-btn" type="button" data-action="bulk-set-project-member-state" data-member-state="candidate"' + projectBulkDisabled + '>Mark Candidate</button>'
        + '  <button class="bulk-btn" type="button" data-action="bulk-set-project-member-state" data-member-state="rejected"' + projectBulkDisabled + '>Mark Rejected</button>';
    }
    var sourceOptions = [
    ...
    if (this._filters && this._filters.show_deleted) {
      return ''
        ...
        + '  <button class="bulk-btn" type="button" data-action="bulk-restore-models"><ha-icon icon="mdi:restore"></ha-icon> Restore</button>'
        + '  <button class="bulk-btn danger" type="button" data-action="bulk-purge-models"><ha-icon icon="mdi:delete-forever-outline"></ha-icon> Purge</button>'
  ...
  _renderModelCard(model) {
    ...
    var isDeletedView = !!(this._filters && this._filters.show_deleted);
    var deleteButton = isLocalModel && !isDeletedView
      ...
      : '';
    var deletedLifecycleActions = isLocalModel && isDeletedView
      ? '  <button class="advanced-action primary" type="button" data-action="restore-model" data-model-ref="' + this._escapeHtml(modelRef) + '" data-local-model-id="' + this._escapeHtml(localModelId) + '" data-model-name="' + this._escapeHtml(name) + '"><ha-icon icon="mdi:restore"></ha-icon><span>Restore model</span></button>'
        + '  <button class="advanced-action danger" type="button" data-action="purge-model" data-model-ref="' + this._escapeHtml(modelRef) + '" data-local-model-id="' + this._escapeHtml(localModelId) + '" data-model-name="' + this._escapeHtml(name) + '"><ha-icon icon="mdi:delete-forever-outline"></ha-icon><span>Purge permanently</span></button>'
      : '';
  ...
  _doRender() {
    ...
    if (!this._persistentStyle) {
      ...
      + '.left-nav{display:grid;gap:10px;padding:12px;border:1px solid var(--line);border-radius:16px;background:var(--surface-1);max-height:calc(100vh - 112px);overflow:auto;scrollbar-gutter:stable;}'
      ...
      + '.project-member-pending{font-size:11px;font-weight:800;color:var(--secondary-text-color);text-transform:uppercase;letter-spacing:.03em;}'
      + '.project-member-state-group{display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;}'
      + '.project-member-state-btn{min-height:30px;padding:0 10px;border-radius:999px;border:1px solid var(--chip-line);background:rgba(15,23,42,0.08);color:var(--secondary-text-color);font-size:11px;font-weight:800;cursor:pointer;text-transform:uppercase;letter-spacing:.03em;}'
      ...
      + '.project-detail-group-count{display:inline-flex;align-items:center;justify-content:center;min-width:28px;height:28px;padding:0 8px;border-radius:999px;border:1px solid var(--chip-line);background:rgba(15,23,42,0.08);font-size:11px;font-weight:800;color:var(--secondary-text-color);}'
      + '.project-detail-group-grid{display:grid;gap:12px;}'
      ...
      + '.results.view-list .model-card{content-visibility:auto;contain-intrinsic-size:auto 108px;}'
      ...
      + '.collection-card{display:grid;gap:10px;padding:14px;border-radius:18px;border:1px solid var(--line);background:rgba(15,23,42,0.18);box-shadow:0 8px 22px rgba(15,23,42,0.16);align-content:start;min-width:0;}'
      ...
      + '.collection-mosaic-tile img[data-thumbnail-lazy-url]:not([src]){font-size:0;color:transparent;background:rgba(148,163,184,0.16);}'
      + '.collection-mosaic-tile:has(img[data-thumbnail-lazy-url]:not([src]))::after{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.07) 50%,transparent);transform:translateX(-100%);animation:shimmer-slide 1.25s ease-in-out infinite;}'
      ...
      + '.collection-pile-photo.pile-5{left:92px;top:96px;transform:rotate(-12deg);}'
      + '.collection-pile-photo.pile-6{left:192px;top:96px;transform:rotate(6deg);}'
      ...
      + '.model-card{position:relative;min-width:0;border-radius:20px;border:1px solid var(--line);background:rgba(15,23,42,0.18);overflow:visible;display:grid;cursor:pointer;transition:border-color .18s ease;contain:layout paint style;}'
      ...
      + '.model-card:hover{border-color:var(--accent-strong);box-shadow:0 2px 8px rgba(15,23,42,0.12);}'
      ...
      + '.thumb,.media-preview{position:relative;display:flex;align-items:center;justify-content:center;background:rgba(15,23,42,0.24);overflow:hidden;}'
      ...
      + '.thumb img[data-thumbnail-lazy-url]:not([src]),.media-preview img[data-thumbnail-lazy-url]:not([src]){background-size:100% 100%;animation:none;}'
      + '.thumb:has(img[data-thumbnail-lazy-url]:not([src]))::after,.media-preview:has(img[data-thumbnail-lazy-url]:not([src]))::after{content:"";position:absolute;inset:0;pointer-events:none;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.07) 50%,transparent);transform:translateX(-100%);animation:shimmer-slide 1.25s ease-in-out infinite;}'
      + '.results.view-list .thumb:has(img[data-thumbnail-lazy-url]:not([src]))::after{animation:none;display:none;}'
      ...
      + '.skeleton-block{position:relative;overflow:hidden;background:rgba(148,163,184,0.10);border-radius:10px;}'
      + '.skeleton-block::after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.06) 50%,transparent);transform:translateX(-100%);animation:shimmer-slide 1.25s ease-in-out infinite;}'
      + '.results.view-list .skeleton-block::after{animation:none;display:none;}'
      ...
      + '.collection-action-backdrop{position:fixed;inset:0;z-index:30;display:flex;align-items:center;justify-content:center;padding:20px;background:rgba(2,6,23,0.82);}'
      ...
      + '@media (prefers-reduced-motion: reduce){.model-card,.toolbar-btn,.toolbar-icon-btn,.view-mode-item,.filter-chip,.selected-filter-chip,.queue-action,.favorite-action,.bulkbar .bulk-btn,.page-control-strip.multi-select-active .bulk-btn,.left-nav,.left-nav-backdrop{transition:none !important;}.thumb::after,.media-preview::after,.collection-mosaic-tile::after,.skeleton-block::after,.refresh-btn.spinning ha-icon{animation:none !important;}.model-card:hover,.queue-action:hover,.queue-action.has-queue-entries:hover{box-shadow:none !important;transform:none !important;}.collection-action-backdrop{background:rgba(2,6,23,0.86);}}'
