...
|------|---------|-------------|
| [officetouch5.yaml](officetouch5.yaml) | `/config/openhasp/officetouch5.yaml` | Defines the object-to-entity mappings for the `officetouch5` plate. Maps JSONL UI objects on the ESP32 to Home Assistant sensor values via the OpenHASP custom component. |
...
- Template helper sensors defined in [template_sensors.yaml](template_sensors.yaml) — must be created in HA before the plate config will work correctly