...
| [#559](https://github.com/rsocko/hass-bambulab-config/issues/559) | Tweak the 'segment' progress to be more 'segments' | **Already implemented.** Segment count was increased from 12 → 20 and animation timing slowed (delay per segment 0.12s → 0.18s, pulse cycle 1.8s → 2.6s) in commit `d11e318`. Applied across all segmented progress card options (10, 11, 13, 14, 15). No further changes needed. |
