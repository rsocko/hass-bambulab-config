...
3. **PR C — endpoints respond 410 Gone** ✅ *complete*: legacy `/api/working-groups/*` endpoints and `/api/working-files/explorer` now return HTTP 410 with a JSON envelope (`success: false`, `error: "endpoint_gone"`, `new_endpoints: [...]`) and `Deprecation: true` + `Link: </api/working-files/tree>; rel="successor-version"` response headers. The new endpoints from design §6.4 (`/api/working-files/tree`, `/loose`, `/groups/{folder_slug}`, `/groups/{folder_slug}/files?mode=files|folders`) ship in the same PR. Covered by `tests/sidecars/model_catalog/test_working_files_folder_endpoints.py` (31 cases: tree/loose/group-detail/group-files happy paths + edge cases for empty root, missing root config, missing folder, invalid mode, invalid slug, plus parametrized 410 assertions for all 19 legacy method/path combinations). NOTE: this breaks the live Working Files Explorer card until PR D lands; that is the intended sequencing.
