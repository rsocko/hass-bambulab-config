...
> **Related design docs:** [catalog-popup-design.md](catalog-popup-design.md), [working-files-card-design.md](working-files-card-design.md), [working-files-workflow-redesign-issue-1169.md](working-files-workflow-redesign-issue-1169.md).

---

## 1. Why a separate "Advanced Actions" card

The existing model-detail popup (see [catalog-popup-design.md](catalog-popup-design.md)) is a **read-and-act-quickly** surface — it shows photos, key fields, related archives, and a small action row. Issue #1227 calls out the gap: there is no single place to perform the **less-frequent, higher-stakes** model operations:
...
4. **Where does the entry button live?** On the model-detail popup ([catalog-popup-design.md](catalog-popup-design.md)), as a `mdi:dots-horizontal` overflow icon in the top-right action row, mirroring the PH browser card pattern. Optionally also on each catalog browser card (compact / list / media) as the `⋯` overflow — same pattern proposed in [working-files-card-design.md §3.7](working-files-card-design.md).
