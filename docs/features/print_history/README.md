...
5. **Archive detail popup is live and now partially actionable** — each archive card opens a `browser_mod.popup`; favorites can be toggled from the card and popup, and popup-backed `tags` / `notes` edits can be saved. `print_name`, compare, and richer follow-on actions remain deferred.
