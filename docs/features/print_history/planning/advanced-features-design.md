...
Useful for voice assistant integration, note/file-name lookup, or a later explicit advanced-search path, but it is not a drop-in replacement for the shipped browser text box.

Current recommendation from issue `#840`:

- keep the existing browser text box on the local search contract
- do not add a standard-vs-full-text toggle to the main browser controls right now
- if this lands later, expose it as an explicit secondary capability rather than silently changing existing search semantics
