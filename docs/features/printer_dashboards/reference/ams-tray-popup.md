...
| ~~Reset Tray~~        | **Implemented** — clears filament metadata via `script.reset_tray_filament` with confirmation dialog; available in both matched-spool and no-spool popups. See [Reset Tray Filament](/docs/features/spoolman_sync/design/reset-tray-filament.md) |
