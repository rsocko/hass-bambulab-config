...
### 10. Bottom Row — More Details, Open in Spoolman, Reload, Reset & Close
`custom:layout-card` with `grid-template-columns: 1fr 1fr 1fr 1fr 1fr` containing five `custom:button-card` buttons side-by-side:
...
| Mismatch threshold config UI | **Partially implemented** — the warning threshold now reads from `input_number.ams_spool_weight_mismatch_threshold_percent`. Remaining work is to surface that helper in a dedicated dashboard config page or popup instead of relying on HA's generic Helpers UI. |
| ~~Reset Tray~~        | **Implemented** — clears filament metadata via `script.reset_tray_filament` with confirmation dialog; available in both matched-spool and no-spool popups. See [reset-tray-filament-design.md](\docs\features\spoolman_sync\design\reset-tray-filament.md) |
