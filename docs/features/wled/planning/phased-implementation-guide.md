...
- Replaces: docs/features/wled/phased-implementation-guide.md
- Replaced By: n/a


> **Updated 2026-03-13** â€” Restructured around the Home Assistant State Machine approach.

## Overview

This guide describes a **6-phase** implementation approach aligned with the deployed HA state machine architecture. Phase 1 is complete; Phases 2-6 progressively close remaining gaps from [light-scenarios.md](../design/light-scenarios.md).

### Architecture Summary

```
Printer Entities â†’ Orchestrator Automation â†’ Transition Script â†’ Core State Helper â†’ Apply Presets Script â†’ WLED Controllers
```

The orchestrator watches printer status entities, computes E_* events, transitions the state machine, and applies WLED presets. See [ha-state-machine-package.md](../reference/ha-state-machine-package.md) for the full state diagram and entity mappings.

### Target Vision

The end goal is a fully-functioning LED system implementing all 33+ scenarios defined in [light-scenarios.md](../design/light-scenarios.md), including:
- Priority-tiered overlays (T0 safety â†’ T5 aesthetic)
...
- [x] Read [ha-state-machine-package.md](../reference/ha-state-machine-package.md)
- [x] Read [controller-allocation.md](../reference/controller-allocation.md)
- [x] Read [light-scenarios.md](../design/light-scenarios.md) â€” master scenario catalog and target behavior spec
...
| `scripts/wled_3dprinter_transition_from_event-script.yaml` | Event â†’ state mapping |
| `scripts/wled_3dprinter_apply_core_state_to_presets-script.yaml` | State â†’ preset mapping |
...
| `helpers/input_select/wled_3dprinter_core_state.yaml` | Current state (S0â€“S8) |
...
| File | Purpose |
|------|---------|
| `wled_state_machine_presets_Digquad_skeleton.json` | Phase 1 presets 101â€“109 (2-segment only, rollback reference) |
| `wled_state_machine_presets_Digquad.json` | Current presets 100â€“109 (15-segment, full layout + Phase 3.1 progress) |
| `wled_state_machine_preset_map.json` | Reference: state â†’ preset ID |
...
| 2â€“4 | Present but not styled by skeleton presets | Seg 2: Front Door Top (status), Seg 3: AMS1 Combined, Seg 4: AMS2 Combined |
| 5â€“14 | N/A | Seg 5â€“8: Tag A1â€“A4, Seg 9: AMS1 Backgrounds, Seg 10â€“13: Tag B1â€“B4, Seg 14: AMS2 Backgrounds |

### Phase 1 Validation Checklist

- [x] Helper entities appear in HA Developer Tools â†’ States
...
- [x] `select.dig_quad_v3_preset` changes to match the core state (SM S0â€“S8)
...
| Start print | Begin a print | S2_PREP â†’ S3_PRINTING | SM S2 Prep â†’ SM S3 Printing |
| Pause from UI | Pause button | S4_PAUSED_USER | SM S4 Paused User |
| Resume | Resume button | S3_PRINTING | SM S3 Printing |
| Print done | Wait for completion | S6_FINISHING â†’ S1_IDLE | SM S6 Finishing â†’ SM S1 Idle |
...
| Only segments 0â€“1 styled | Skeleton presets only define 2 segments | Addressed in Phase 2 |

---

## Phase 2: Segment Expansion + Preset Enhancement

**Goal**: Deploy the full 15-segment layout on DigQuad; expand presets 100â€“109 to style all segments per state  
**Status**: Implementation ready (files created, pending deployment)  
**Prerequisite**: Phase 1 validated  
**Design reference**: [light-scenarios.md](../design/light-scenarios.md) â€” Sections 3 (scenario catalog), 5 (smart defaults), 6 (baseline presets)
...
|-----|------|------|-----------|-------|---------|
| 0 | Front Door Bottom | 15 | 0â€“49 | 50 | Print progress bar |
| 1 | Front Door Left | 15 | 50â€“114 | 65 | Layer progress |
| 2 | Front Door Top | 15 | 115â€“157 | 43 | Status indicator |
| 3 | AMS1 Combined | 1 | 158â€“297 | 140 | AMS 1 tray lighting (top + bottom) |
| 4 | AMS2 Combined | 3 | 298â€“436 | 139 | AMS 2 tray lighting (top + bottom) |
| 5 | Tag A1 Top | 16 | 437â€“453 | 17 | Tray A1 tag (incl. side start) |
| 6 | Tag A2 Top | 16 | 454â€“465 | 12 | Tray A2 tag |
| 7 | Tag A3 Top | 16 | 466â€“489 | 24 | Tray A3 tag (incl. hygrometer) |
| 8 | Tag A4 Top | 16 | 490â€“501 | 12 | Tray A4 tag |
| 9 | AMS1 Tag Bottoms | 16 | 502â€“572 | 71 | AMS1 tag bottoms + backgrounds |
| 10 | Tag B1 Top | 4 | 573â€“591 | 19 | Tray B1 tag (incl. side start) |
| 11 | Tag B2 Top | 4 | 592â€“605 | 14 | Tray B2 tag |
| 12 | Tag B3 Top | 4 | 606â€“631 | 26 | Tray B3 tag (incl. hygrometer) |
| 13 | Tag B4 Top | 4 | 632â€“643 | 12 | Tray B4 tag |
| 14 | AMS2 Tag Bottoms | 4 | 644â€“710 | 67 | AMS2 tag bottoms + backgrounds |
...
- AMS top + bottom combined per unit (2 segments instead of 4) â€” minimal visual differentiation at this phase
- Side-wall and hygrometer LEDs absorbed into adjacent tag/background segments â€” Phase 3 can differentiate via API overlays
...
2. Merge `wled_state_machine_presets_Digquad.json` into DigQuad (this file contains presets 100â€“109)
...
5. Test each state preset (101â€“109) by selecting them in the WLED UI

### 2.2: Expand State Machine Presets

New file: `wled_state_machine_presets_Digquad.json` â€” replaces the Phase 1 skeleton with full 15-segment presets.  
...
**Presets 101â€“109**: Style all 15 segments per state (no boundary changes â€” references existing segment IDs).

**Design guidance per state**:

| State | Seg 0 (Progress) | Seg 1 (Left) | Seg 2 (Top/Status) | Seg 3â€“4 (AMS) | Seg 5â€“8, 10â€“13 (Tags) | Seg 9, 14 (Backgrounds) |
|-------|-------------------|--------------|---------------------|----------------|------------------------|-------------------------|
...
**Steps**:
1. Merge `wled_state_machine_presets_Digquad.json` into DigQuad's presets (replaces 101â€“109, adds 100)
...
**Test procedure**:
1. Open HA Developer Tools â†’ Services
...
| Printing look | S3_PRINTING | Segs 0â€“2 green; AMS white; tags/backgrounds soft white |
| Paused user | S4_PAUSED_USER | All segments yellow; seg 2 blinking |
| Error look | S5_PAUSED_ERROR | All segments red; segs 0â€“2 blinking |
| Finishing look | S6_FINISHING | Segs 0â€“1 green; seg 2 green wipe; AMS/tags lit |
...
**Rollback**: If segment expansion causes issues, re-upload the original skeleton presets (101â€“109 with segments 0â€“1 only) from the backup.
...
**Design reference**: [light-scenarios.md](../design/light-scenarios.md) â€” Sections 4 (segment limits), 8 (hybrid control), 10 (priority tiers), 11 (state machine overlays), 12 (idle rotation)

### 3.1: Progress Bar & Status Enhancement (âœ… IMPLEMENTED)
...
|---------|------|--------|-------|-------------|
| 0 | Front Door Bottom | Percent (fx 98) | Green fill `[0, 255, 40]` on dim white `[50, 44, 36]` background | `sensor.ntk_ryansoffice_3dprinter_print_progress` (0â€“100%) |
...
| 2 | Front Door Top | Breathe (fx 2), very slow (sx 20) | Green `[0, 200, 60]` | N/A â€” indicates healthy print in progress |
...
4. The overlay script sends a targeted WLED JSON API request (via `rest_command.wled_digquad_update_state`) that updates ONLY the `ix` parameter on segments 0 and 1 â€” all other segment properties remain as set by preset 104
...
**ix mapping** (Percent effect uses 0â€“100 direct percentage):
- Segment 0: `ix = round(print_progress)` â†’ 0=empty, 100=full
- Segment 1: `ix = round(current_layer / total_layers Ã— 100)` â†’ 0=empty, 100=full (safe division, 0 when total_layers is 0)

#### Files Created / Modified

| File | Change |
|------|--------|
| `helpers/input_text/wled_3dprinter_digquad_ip.yaml` | **New** â€” Input text helper for DigQuad WLED IP address (required for JSON API calls) |
| `rest_commands/wled_3dprinter_digquad_update_state.yaml` | **New** â€” Generic REST command to POST JSON to WLED `/json/state` endpoint |
| `scripts/wled_3dprinter_apply_progress_overlay-script.yaml` | **New** â€” Reads progress sensors, calculates ix, sends segment update via REST |
| `automations/wled_3dprinter_progress_overlay.yaml` | **New** â€” Triggers on progress/layer sensor changes; conditions on S3_PRINTING |
| `automations/wled_3dprinter_state_machine_orchestrator.yaml` | **Modified** â€” Added progress overlay call after S3_PRINTING preset application |
| `wled_loader.yaml` | **Modified** â€” Added `rest_command: !include_dir_merge_named rest_commands` |
| `wled_state_machine_presets_Digquad.json` (preset 104) | **Modified** â€” Seg 0: Percent fx green/white, Seg 1: Percent fx blue/white, Seg 2: slow green Breathe |

#### Setup Steps

1. Set `input_text.wled_3dprinter_digquad_ip` to the DigQuad's IP address (e.g., `192.168.1.xx`) in HA â†’ Settings â†’ Helpers
2. Verify `rest_command.wled_3dprinter_digquad_update_state` is available (HA â†’ Developer Tools â†’ Services)
3. Verify the Percent effect ID on your WLED build â€” effect ID **98** is confirmed for DigQuad at `192.168.50.103`. Check WLED UI â†’ Effects or query `http://<ip>/json/effects` to verify. If different, update preset 104 `fx` values for segments 0 and 1.
...
| Test | Expected |
|------|----------|
| Enter S3_PRINTING | Seg 0 and 1 show dim white base (Percent at ixâ‰ˆ0), Seg 2 slow green breathe |
...
1. Create a new script `wled_3dprinter_apply_active_tray_overlay` that:
   ...
   - Uses the WLED API to override the matching tag segment (5â€“8, 10â€“13) color
...
| File | Change |
|------|--------|
| `helpers/input_number/wled_3dprinter_idle_tray_risk_threshold_grams.yaml` | **New** â€” low-filament threshold helper for idle tray risk overlays |
| `helpers/input_number/wled_3dprinter_idle_humidity_warning_threshold.yaml` | **New** â€” humidity threshold helper for idle AMS background warnings (default 50% to match dashboard red threshold) |
| `scripts/wled_3dprinter_apply_idle_telemetry_overlays-script.yaml` | **New** â€” idle overlay coordinator (preset reset + ordered overlay application) |
| `scripts/wled_3dprinter_apply_idle_tray_risk_overlay-script.yaml` | **New** â€” tray risk overlay script with dashboard-matched thresholds |
| `scripts/wled_3dprinter_apply_idle_humidity_overlay-script.yaml` | **New** â€” humidity warning overlay script for AMS background segments |
| `scripts/wled_3dprinter_apply_idle_desiccant_overlay-script.yaml` | **New** â€” desiccant-age overlay script per tray/tag |
| `automations/wled_3dprinter_idle_telemetry_overlays.yaml` | **New** â€” idle telemetry trigger automation |
| `automations/wled_3dprinter_state_machine_orchestrator.yaml` | **Modified** â€” calls idle overlay coordinator when state is `S1_IDLE` |
| `wled_loader.yaml` | **Modified** â€” adds `input_number` helper loader |
...
| Phase 2 | Re-upload skeleton presets (101â€“109 segment 0+1 only) from `wled/backups/digquad/2026-03-13 - 2 - Phase 1 Implemented/` or `wled_state_machine_presets_Digquad_skeleton.json` |
| Phase 3.1 | Remove progress overlay automation + script + rest_command; revert preset 104 segs 0â€“2 to solid green; remove orchestrator progress overlay call |
...
### Phase 1 (Core State Machine) â€” Complete
...
- [ ] Preset 100 (SM Base Layout) loaded â€” 15 segments established
- [ ] All 15 segments visible and addressable in WLED UI
- [ ] Each preset (101â€“109) styles all 15 segments appropriately
- [ ] Manual state override from Developer Tools works
- [ ] Full print lifecycle looks correct (idle â†’ prep â†’ printing â†’ finishing â†’ idle)
...
- [ ] S3 Printing: segs 0â€“2 green, AMS white, tags soft white
...
- [x] `rest_command.wled_3dprinter_digquad_update_state` reachable (Developer Tools â†’ Services)
...
| Event stuck on E_NOOP? | Check `input_text.wled_3dprinter_last_event` â€” if E_NOOP, the smart_status may be an unmapped value |

### DigQuad Not Changing Presets
| Check | How |
|-------|-----|
| Preset entity? | `select.dig_quad_v3_preset` should show available options |
| Preset names match? | The apply script sends exact preset name strings like "SM S3 Printing" â€” verify they match installed presets |
...
1. Go to HA â†’ Settings â†’ Automations
...
**Version**: 3.1 (Roadmap expanded to Phases 4-6 â€” 2026-03-15)  
**Phases**: 6 (Core âœ… â†’ Segment Expansion â†’ Core Overlays âœ…/deferred â†’ Telemetry Completion â†’ Environmental/Utility â†’ Advanced Dynamic)  
**Architecture**: HA State Machine â†’ WLED Presets 100â€“109 + JSON API Overlays (+ optional dynamic segment remap in Phase 6)
