...
- Replaces: docs/features/wled/quick-start-preset-based.md
- Replaced By: n/a


> **STATUS: FUTURE (Phase 3)** â€” This guide describes the preset-based segment approach which is planned for Phase 3. The current system uses the state machine (presets 101â€“109). See [phased-implementation-guide.md](phased-implementation-guide.md).
...
Segment 6: A1 Tag Top (LEDs 442-453) â† ACTIVE TAG TOP
Segment 7: A1 Tag Bottom (LEDs 502-513) â† ACTIVE TAG BOTTOM
...
5. **Save as Preset 50**:
   ...
   - **CRITICAL**: Check "Save segment bounds" option âœ“
...
**Fix**: Check your sensor names in Developer Tools â†’ States
...
Call it from Developer Tools â†’ Services:
```yaml
...
- **Full Documentation**: [preset-based-segments.md](../design/preset-based-segments.md)
- **Automation Examples**: [ha-automation-preset-based.md](../design/preset-based-automation-examples.md)
- **Example Configs**: `digquad-settings/wled_preset_*_full_highlight.json`
- **Main Specification**: [preset-specification.md](../archive/preset-specification-legacy.md)

## Benefits Recap

âœ… **Full Tag Control**: Both top and bottom of active tag  
âœ… **No Hardware Changes**: Pure software solution  
âœ… **Within Segment Limit**: Each preset uses â‰¤16 segments  
âœ… **Dynamic**: Automatically switches based on active tray  
âœ… **Backward Compatible**: Base presets still work  

## Trade-offs

âš ï¸ **Switching Delay**: ~500ms when changing presets  
âš ï¸ **Inactive Tags Combined**: Non-active tags lose individual control  
âš ï¸ **More Presets**: Need to manage 8 configurations (50-57)  
âš ï¸ **Complex Automation**: More sophisticated Home Assistant logic  
