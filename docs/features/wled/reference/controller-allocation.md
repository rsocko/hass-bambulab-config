...
- Replaces: docs/features/wled/controller-allocation.md
- Replaced By: n/a
...
**âš ï¸ CRITICAL CONSTRAINT**: DigQuad has 5 GPIO outputs and all are currently in use. No additional LED strips can be connected to DigQuad.
...
**Minimum Total on DigQuad: 16 segments** (with combined AMS tray lighting â€” uses WLED maximum)
...
**Current Count: 16 segments** (0-15) â€” **uses WLED maximum, 0 spare**
...
#### DigQuad - Segment Allocation (16 segments used, 0 spare â€” WLED maximum)
...
**Total: 16 segments on DigQuad, 0 spare (WLED maximum) âœ…**

#### MagWLED - Segment Allocation (1 segment used, 15 spare)

0. **Segment 0**: Interior Lid Light - 1 segment (simple on/off)

**Total: 1 segment on MagWLED, 15 segments available for future expansion! âœ…**
...
âœ… **Full tag highlighting**: Each of 8 tray tags gets individual control  
âœ… **Print progress bar**: Independent print percentage on front door bottom  
âœ… **Layer progress**: Independent layer progress on front door left  
âœ… **Status indication**: Independent status indicator on front door top  
âœ… **Basic AMS lighting**: Combined top/bottom lighting per AMS unit  
âœ… **Neutral backgrounds**: Tag bottoms and hygrometers have soft white light  

### What Fixed Layout Cannot Do Simultaneously

In the fixed 16-segment DigQuad layout, the following are limited:

âŒ **Per-tray AMS lid lighting** for all trays at once  
âŒ **Individual tag bottom control** for all trays at once  
âŒ **Independent hygrometer control** while also keeping detailed tag-bottom telemetry  
âŒ **All detailed tray-level effects concurrently** (AMS tops + tag tops + tag bottoms + hygrometers)  
...
**Recommendation**: âœ… Recommended as primary advanced strategy when bounded by guardrails
...
**Recommendation**: âš ï¸ Consider if bottom lighting is not valued
...
**Recommendation**: âš ï¸ Consider if tag bottom functions can be moved to tag top
...
**Recommendation**: âœ… Recommended baseline strategy for fixed layout
...
**Recommendation**: âš ï¸ Possible but adds complexity
...
1. âœ… **NO HARDWARE CHANGES NEEDED** - DigQuad is at full capacity (5 GPIO pins)
2. âœ… Interior Lid Light remains on MagWLED
3. âœ… Verify all LED counts match physical installation (711 on DigQuad + ~30 on MagWLED)

### Phase 2: Configuration Updates  
1. âœ… Update DigQuad segment definitions (16 segments used, 0 spare)
2. âœ… Configure 3 independent front door segments (bottom, left, top)
3. âœ… Set up neutral segments with soft white defaults
4. âœ… Keep MagWLED configuration for Interior Lid Light (1 segment, 15 spare)

### Phase 3: Preset Creation
1. âœ… Create presets for all scenarios (see preset-specification.md)
2. âœ… Test each preset to verify segment allocation works
3. âœ… Document workarounds for blocked scenarios
4. âœ… Coordinate presets across both controllers when needed

### Phase 4: Home Assistant Integration
1. âœ… Update automations to use new segment IDs (DigQuad 0-15)
2. âœ… Implement workarounds for blocked scenarios
3. âœ… Configure MagWLED Interior Lid Light control  
3. âœ… Test all automation triggers
...
âœ… Respects DigQuad's 5 GPIO pin limit (no hardware changes needed)
âœ… Uses all 16 segments on DigQuad for maximum functionality  
âœ… 3 independent front door segments (print progress, layer progress, status)
âœ… Maintains individual tag top control (8 tags)  
âœ… Maintains progress bar and layer progress functionality  
âœ… Maintains independent status indication  
âœ… Provides neutral background lighting  
âœ… Keeps MagWLED with 15 segments available for future expansion  
âœ… Simple Interior Lid Light control on MagWLED (1 segment)

### Key Limitations
âš ï¸ Full concurrent fidelity across all tray-level dimensions is not possible in one static 16-segment map  
âš ï¸ Fixed layout degrades per-tray bottom metrics and hygrometer independence  
âš ï¸ Hybrid dynamic/preset strategy is required for high-detail tray-specific moments  

### Hardware Reality
âš ï¸ **CRITICAL**: DigQuad has 5 GPIO pins and ALL are in use:
- GPIO 15: Printer Front Door (158 LEDs)
- GPIO 1: AMS 1 Lid/Spools (140 LEDs)
- GPIO 3: AMS 2 Lid/Spools (139 LEDs)
- GPIO 16: AMS 1 Tags + Hygrometer (136 LEDs)
