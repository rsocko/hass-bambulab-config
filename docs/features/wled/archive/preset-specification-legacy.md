...
- Replaces: docs/features/wled/preset-specification.md
- Replaced By: n/a


> **STATUS: LEGACY** â€” These preset definitions (1â€“49) were a design specification that was **never deployed** to the DigQuad. The active system uses **state machine presets 101â€“109** defined in [ha-state-machine-package.md](../reference/ha-state-machine-package.md). This document is retained as a reference for future preset expansion in Phase 2/3 of the [phased-implementation-guide.md](../planning/phased-implementation-guide.md).
...
## Preset-Based Segment Configuration (Future â€” Phase 3)

This specification also references an advanced technique where WLED presets save segment definitions (not just colors), enabling dynamic 16-segment layout switching based on the active tray. This is a **future Phase 3 capability** â€” see [preset-based-segments.md](../design/preset-based-segments.md) for details.

- **Presets 1-49**: Traditional presets (colors/effects on fixed segment layout) â€” documented below
- **Presets 50-57**: Advanced preset configurations that redefine segment boundaries â€” see [preset-based-segments.md](../design/preset-based-segments.md)
- **Presets 101-109**: Active state machine presets â€” see [ha-state-machine-package.md](../reference/ha-state-machine-package.md)
...
#### DigQuad Controller (16 segments used, 0 spare â€” WLED maximum)
...
| Segment | Color | Effect | Brightness | Notes |
|---------|-------|--------|------------|-------|
| 7 | Filament Color | Solid | 80% | â­ A1 tag highlighted |
...
| 8 | Filament Color | Solid | 80% | â­ A2 tag highlighted |
...
| 9 | Filament Color | Solid | 80% | â­ A3 tag highlighted |
...
| 10 | Filament Color | Solid | 80% | â­ A4 tag highlighted |
...
| Segment | Color | Effect | Brightness | Notes |
|---------|-------|--------|------------|-------|
| 11 | Filament Color | Solid | 80% | â­ B1 tag highlighted |
...
| 12 | Filament Color | Solid | 80% | â­ B2 tag highlighted |
...
| 13 | Filament Color | Solid | 80% | â­ B3 tag highlighted |
...
| 14 | Filament Color | Solid | 80% | â­ B4 tag highlighted |
...
#### Preset 27: Humidity High (âš ï¸ Degraded)
**Scenario**: Hygrometer reading above threshold  
**Trigger**: `sensor.ams_humidity > 60`

**âš ï¸ LIMITATION**: Cannot individually control each AMS hygrometer since they share segment 15.
...
| 27 | Humidity High | Dynamic | Humidity > 60 | Medium (âš ï¸) |
...
## ðŸ†• Advanced: Preset-Based Segment Configurations (Presets 50-57)
...
### Benefits
âœ… **Full Tag Highlighting**: Both top and bottom of active tag show filament color  
âœ… **Within 16-Segment Limit**: Each preset configuration uses â‰¤16 segments  
âœ… **No Hardware Changes**: Pure software solution  
âœ… **Backward Compatible**: Base presets (1-49) continue to work  
âœ… **Dynamic**: Automatically adapts to active tray changes

### Trade-offs
âš ï¸ **Switching Delay**: ~500ms delay when changing preset configurations  
âš ï¸ **Inactive Tags Combined**: Non-active tags lose individual control (acceptable as they're dim)  
âš ï¸ **Preset Management**: Need to maintain 8 preset configurations  
âš ï¸ **Automation Complexity**: Requires more sophisticated Home Assistant logic

### Implementation Resources

For complete implementation details, see:
- **[preset-based-segments.md](../design/preset-based-segments.md)** - Comprehensive guide to preset-based segment configurations
- **[ha-automation-preset-based.md](../design/preset-based-automation-examples.md)** - Home Assistant automation examples
