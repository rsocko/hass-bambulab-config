...
- Replaces: docs/features/wled/summary.md
- Replaced By: n/a


> **Updated 2026-03-13** â€” Reflects the deployed Home Assistant State Machine approach.

## Current Status

The WLED system is controlled by a **Home Assistant state machine** that is live and running:

- **Phase 1 (Core State Machine)**: âœ… Complete â€” 9 core states (S0â€“S8), presets 101â€“109 on DigQuad, HA package with orchestrator + scripts + helpers
- **Phase 2 (Segment Expansion)**: Not started â€” deploy full 15-segment layout and expand presets
- **Phase 3 (Overlays & Advanced)**: In progress â€” 3.1, 3.2, and 3.4 implemented; 3.3 deferred (optional preset-based switching)

See [ha-state-machine-package.md](../reference/ha-state-machine-package.md) for the authoritative state diagram and entity mapping. See [phased-implementation-guide.md](../planning/phased-implementation-guide.md) for the 3-phase plan.
...
> **Note**: This technique is a valid future enhancement for Phase 3. It has not been deployed yet. The current system uses the state machine with presets 101â€“109.
...
### What This Enables
âœ… **Full Tag Highlighting**: Control BOTH top AND bottom of active tag with filament color  
âœ… **Dynamic Layouts**: Switch between different 16-segment configurations on the fly  
âœ… **No Hardware Changes**: Pure software solution via preset switching  
âœ… **Context-Aware**: Automatically adapt segment layout based on active tray
...
### Documentation
- **[preset-based-segments.md](../design/preset-based-segments.md)** - Complete conceptual guide
- **[ha-automation-preset-based.md](../design/preset-based-automation-examples.md)** - Automation examples
...
1. âœ… **Controller allocation advice** - Which controller (MagWLED vs DigQuad) should control each LED strip
2. âœ… **Merge front door segments** - Combine left and top segments to free up a segment
3. âœ… **Preset specification** - Document all presets with scenarios for different active trays
4. âœ… **Segment limitation analysis** - Identify blocked scenarios and alternatives
5. âœ… **Neutral segments** - Define segments for unused areas with soft white
6. âœ… **Phased implementation** - Create a phased approach for testing and validation

## Key Deliverables

### 1. Controller Allocation Recommendation
**Document**: [controller-allocation.md](../reference/controller-allocation.md)
...
**Segment Breakdown**:
- **DigQuad (16 segments, 0 spare â€” WLED maximum)**:
...
- **Total: 17 active segments (16+1) âœ…**

### 2. Preset Specification
**Document**: [preset-specification.md](preset-specification-legacy.md)
...
**Document**: [phased-implementation-guide.md](../planning/phased-implementation-guide.md)

**3 Implementation Phases** (aligned to the state machine approach):
1. **Phase 1**: Core State Machine (âœ… Complete) â€” HA package, presets 101â€“109
2. **Phase 2**: Segment Expansion â€” deploy 15 segments, expand presets
3. **Phase 3**: Overlays & Advanced â€” active tray highlighting, telemetry overlays
...
- âœ… **Individual tag highlighting**: Each of 8 tray tags gets individual control
...
- âœ… **All 31+ presets**: Complete scenario coverage

### What Static Fixed Layout Cannot Do Concurrently âŒ

- âŒ **Per-tray AMS lid lighting for all trays simultaneously**
- âŒ **Individual tag bottom control for all trays simultaneously**
- âŒ **Independent hygrometer alerts while preserving all other tray-level details**
- âŒ **Full concurrent tray-detail fidelity in one static 16-segment map**
...
1. âœ… Test incrementally (one phase at a time)
2. âœ… Validate each phase before moving to next
3. âœ… Create backups before major changes
4. âœ… Document any deviations from plan
5. âœ… Test with actual print jobs
...
### Technical Benefits
- âœ… Respects DigQuad's 5 GPIO pin limit (no hardware changes)
- âœ… Stays within 16-segment limit per controller (16 on DigQuad, 1 on MagWLED)
- âœ… Maintains excellent functionality
- âœ… Individual control of all 8 tray tags
- âœ… Print progress bar, layer progress, and status indication
- âœ… Simplified wiring (each controller manages its own strips)
- âœ… MagWLED has 15 segments available for future expansion
- âœ… Individual control of all 8 tray tags
- âœ… Print progress bar, layer progress, and status indication
- âœ… Simplified wiring (single controller)
- âœ… Room for future expansion (MagWLED available)

### Implementation Benefits
- âœ… Phased approach allows incremental testing
- âœ… Early MVP delivery (Phases 1-3)
- âœ… Clear validation checkpoints
- âœ… Documented rollback procedures
- âœ… Comprehensive testing checklist

### Maintenance Benefits
- âœ… Well-documented configuration
- âœ… Clear preset specifications
...
- âœ… Segment count verified (16 on DigQuad + 1 on MagWLED = 17 segments)
- âœ… LED count verified (711 LEDs)
- âœ… All scenarios mapped to presets
- âœ… Workarounds documented for blocked scenarios
...
- âœ… Individual tag highlighting for all 8 trays
- âœ… Basic AMS lighting
- âœ… Error states
- âœ… Loading/unloading animations
...
**Key Documents**:
- [controller-allocation.md](../reference/controller-allocation.md)
- [preset-specification.md](preset-specification-legacy.md)
- [phased-implementation-guide.md](../planning/phased-implementation-guide.md)
