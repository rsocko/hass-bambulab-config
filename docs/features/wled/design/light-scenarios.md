...
- Replaces: docs/features/wled/light-scenarios.md
- Replaced By: n/a
...
| Scenario                           | Priority | Recommended behavior                                                                     | Granularity needed                         | Limit risk | Implementation status (2026-03) | Outstanding / notes |
| ---------------------------------- | -------- | ---------------------------------------------------------------------------------------- | ------------------------------------------ | ---------- | ------------------------------- | ------------------- |
| Printer offline / unreachable      | P2       | Door dim amber, most other zones off                                                     | Global + door status                       | Low        | Implemented                     | Mapped state/preset: `S0_OFFLINE` / `101` (`E_OFFLINE`). |
| Printer idle / ready (reset state) | P2       | Neutral white base, soft blue door status                                                | Global + door status                       | Low        | Implemented                     | Mapped state/preset: `S1_IDLE` / `102` (`E_IDLE`). |
| Printer busy (general non-idle)    | P2       | Medium white + state-color door                                                          | Global + door status                       | Low        | Partial                         | Mapped states/presets: `S2/S3/S4/S5/S6/S7` / `103/104/105/106/107/108`; no distinct generic busy preset. |
| Heating bed                        | P2       | Orange emphasis (door pulse)                                                             | Door status + optional lid                 | Low        | Partial                         | Mapped state/preset: `S2_PREP` / `103`; sub-stage not separated from other prep states. |
| Heating nozzle                     | P2       | Yellow emphasis (door pulse)                                                             | Door status + optional lid                 | Low        | Partial                         | Mapped state/preset: `S2_PREP` / `103`; no dedicated nozzle-heating preset. |
| Bed leveling / lidar               | P2       | Blue pulse/chase                                                                         | Door status                                | Low        | Partial                         | Mapped state/preset: primarily `S2_PREP` / `103` (and some maintenance stages in `S7` / `108`); no dedicated leveling/lidar preset. |
| Purge line / nozzle cleaning       | P2       | Cyan pulse                                                                               | Door status                                | Low        | Partial                         | Mapped states/presets: `Nozzle Prep` -> `S2` / `103`, `cleaning_nozzle_tip` -> `S7` / `108`; no cyan-specific preset. |
| Printing (base)                    | P2       | Door progress + green status + active tray color                                         | Door progress + per-tag-top                | Medium     | Implemented                     | Mapped state/preset: `S3_PRINTING` / `104` plus active overlays: progress (`seg0/1`) and active tray color (`seg5-8`,`seg10-13`). |
| Print paused (user)                | P2       | Yellow blink/pulse                                                                       | Door status + optional global              | Low        | Implemented                     | Mapped state/preset: `S4_PAUSED_USER` / `105` (`E_PAUSE_USER`). |
| Print paused (error)               | P1       | Red strobe + affected tray red                                                           | Door status + per-tag-top                  | Medium     | Partial                         | Mapped state/preset: `S5_PAUSED_ERROR` / `106` (`E_PAUSE_ERROR`); affected-tray targeting not mapped. |
| Print finished                     | P2       | Green pulse then return to ready                                                         | Door status + optional used-tray highlight | Low        | Implemented                     | Mapped state/preset: `S6_FINISHING` / `107`, then `S1_IDLE` / `102`. |
| Cooling down / fans post-print     | P3       | Dim blue                                                                                 | Global or door status                      | Low        | Not implemented                 | No dedicated mapped state/preset; `Cooling Chamber` is currently folded into `S2_PREP` / `103`. |
| Filament runout                    | P1       | Door red alert + affected tray red                                                       | Per-tag-top targeting                      | Medium     | Partial                         | Mapped state/preset: `S5_PAUSED_ERROR` / `106` via runout token detection; per-tray mapping missing. |
| Filament tangle / jam              | P1       | Orange strobe, active tray emphasis                                                      | Per-tag-top targeting                      | Medium     | Partial                         | Mapped state/preset: `S5_PAUSED_ERROR` / `106` via clog/jam-like tokens; no dedicated jam preset. |
| AMS communication error            | P1       | Purple AMS alert + door status                                                           | Per-AMS grouping preferred                 | Medium     | Partial                         | Mapped state/preset: `S5_PAUSED_ERROR` / `106` via `ams lost`; no dedicated AMS-error preset. |
| Temperature fault                  | P1       | Global red/high visibility                                                               | Global + door status                       | Low        | Partial                         | Indirectly mapped via fault pauses to `S5_PAUSED_ERROR` / `106`; no direct temp-fault mapping. |
| Door open during print             | P1       | Bright white visibility and caution pattern                                              | Door + lid/global                          | Low        | Not implemented                 | No mapped state/preset; door sensor is not consumed by orchestrator. |
| Filament loading                   | P2       | Blue chase + active tray tag                                                             | Per-tag-top + optional AMS top             | Medium     | Partial                         | Mapped state/preset: `S7_MAINTENANCE` / `108` for `filament_loading`; no tray-specific overlay mapping. |
| Filament unloading                 | P2       | Teal chase + active tray tag                                                             | Per-tag-top + optional AMS top             | Medium     | Partial                         | Mapped state/preset: `S7_MAINTENANCE` / `108` for `filament_unloading`; no tray-specific overlay mapping. |
| AMS drying mode                    | P3       | Warm amber AMS + bright hygrometer                                                       | Per-AMS + hygrometer                       | High       | Not implemented                 | No mapped state/preset for drying mode in current orchestrator. |
| AMS humidity high                  | P1       | Red hygrometer + AMS pulse                                                               | Per-AMS hygrometer ideal                   | High       | Partial                         | Idle-only humidity threshold overlay implemented in 3.4 (`seg9`,`seg14`); no dedicated hygrometer segment control and no non-idle humidity alert path. |
| AMS humidity normal                | P3       | White hygrometer/base                                                                    | Per-AMS hygrometer ideal                   | High       | Partial                         | Baseline state presets provide neutral visuals; 3.4 adds idle-only humidity warning overlays rather than explicit hygrometer-normal scene control. |
| AMS tray selected (pre-print)      | P2       | Selected tray in filament color                                                          | Per-tag-top                                | Medium     | Not implemented                 | No mapped state/preset; no pre-print tray-selection overlay automation. |
| AMS tray actively feeding          | P2       | Bright active tray + subdued others                                                      | Per-tag-top                                | Medium     | Partial                         | Active tray tag-top highlight implemented in 3.2 for `S3_PRINTING`; no dedicated feed-stage distinction and no per-AMS-top differentiation. |
| Active spool (A1..B4 variants)     | P2       | Highlight active tag top and corresponding spool zone                                    | Per-tag-top + per-tray AMS top is ideal    | High       | Partial                         | Active spool tag-top highlight implemented in 3.2 (`seg5-8`,`seg10-13`); corresponding per-tray AMS-top zone highlighting not implemented. |
| Spools used in current print       | P3       | Highlight all participating trays with shortage-risk color logic from AMS tray dashboard | Multi-tag-top                              | High       | Not implemented                 | No mapped state/preset; multi-tray risk overlay logic not implemented. |
| Spool empty (multi-tray possible)  | P1       | Highlight one or more affected trays simultaneously                                      | Multi-tag-top simultaneous                 | High       | Partial                         | Idle-only tray-risk overlay (3.4) can mark multiple trays from remaining-weight thresholds; explicit active-print multi-tray empty targeting is still missing. |
| Spool desiccant age warning        | P3       | Age color bands (yellow/orange/red), idle rotation scene only                            | Tag bottom per tray ideal                  | Medium     | Partial                         | Idle-only desiccant overlay implemented in 3.4 from `sensor.spoolman_tray_map` status; rendered on tag-top segments (not per-tag-bottom control). |
| Chamber light manual mode          | P3       | White visibility preset                                                                  | Global                                     | Low        | Partial                         | No dedicated mapped state/preset; manual control exists outside state machine (dashboard light entities). |
| Nozzle cleaning required           | P3       | Orange maintenance pulse                                                                 | Door status + optional lid                 | Low        | Partial                         | Mapped state/preset: `S7_MAINTENANCE` / `108` when `cleaning_nozzle_tip`; no separate alert preset. |
| High chamber temperature           | P1       | Red pulse warning                                                                        | Door/lid/global                            | Low        | Not implemented                 | No mapped state/preset; no chamber-temperature trigger path in orchestrator. |
| Low chamber temperature            | P3       | Blue cool warning                                                                        | Door/lid/global                            | Low        | Not implemented                 | No mapped state/preset; no low-temperature trigger path in orchestrator. |
| Power loss recovery                | P1       | Purple recovery pulse                                                                    | Door/lid/global                            | Low        | Not implemented                 | No mapped state/preset for recovery events in current WLED package. |
| Remote monitoring mode             | P3       | Bright white for camera                                                                  | Global/lid + door                          | Low        | Not implemented                 | No mapped state/preset; no helper/automation for remote-monitor mode. |
| Night mode                         | P3       | Very dim warm white or off                                                               | Global                                     | Low        | Not implemented                 | No mapped state/preset; no scheduled/manual night-mode automation in state machine. |
| Show mode                          | P4       | Aesthetic motion pattern                                                                 | Global + optional AMS effects              | Medium     | Implemented                     | Mapped state/preset: `S8_SHOW` / `109` via `input_boolean.wled_3dprinter_show_mode_enabled`. |
...
| AMS1 and AMS2 independent hygrometer alerts | Degraded | Supported | 3.4 currently provides idle-only per-AMS background alerts (`seg9`,`seg14`), not dedicated hygrometer segment control |
