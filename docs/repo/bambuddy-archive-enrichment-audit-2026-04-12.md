...
|---|---|---|---|---|---|---|
| 185 | Boston 3D Miniature v2 (water) - Plate 2 | completed | missing | C | No hidden enrichment payload is stored. | Queue for manual re-enrich/backfill after workflow improvements land. |
| 187 | Boston 3D Miniature v2 - Frame | completed | missing | C | No hidden enrichment payload is stored. | Queue for manual re-enrich/backfill after workflow improvements land. |
...
| 204 | Game of Thrones Poster - Hueforge Art | failed | missing | C | Failed archive has no hidden enrichment payload. | Backfill only if the archive remains useful; otherwise leave as a failure-only record. |
